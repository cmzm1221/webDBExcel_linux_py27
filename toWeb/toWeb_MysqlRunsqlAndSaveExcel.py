...
from lib.mysqlRunSqlAndSaveExcel import RunSqlAndSaveExcel
from lib.base.util import Util


class ToWeb_MysqlRunSqlAndSaveExcel():
    ...
    def toWeb_runSqlAndSaveExcel(self, fileName):
        ...
        try:
            ...
            if fileName[-3:] in ('sql', 'txt'):   # 判断文件类型
                ...
                rqse = RunSqlAndSaveExcel(db_host=db_host, port=port, user=user, password=password)
                # rqse.runSQLAndOnlySaveNewExcel 返回结果 {'errno':0, 'errmsg':'', 'save_excel_name':''}
                res_rssw = rqse.runSQLAndOnlySaveNewExcel(sql_path_name, db_name)
                print res_rssw
